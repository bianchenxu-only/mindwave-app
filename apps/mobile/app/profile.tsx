...
const insightCards = [
  { label: '情绪曲线', value: '平静上升', note: '最近 7 天焦虑标签减少 18%', icon: 'pulse-outline' },
  { label: '睡眠趋势', value: '82 分均值', note: '本周最佳：周二雨声入眠', icon: 'moon-outline' },
  { label: '专注统计', value: '8.5h', note: '最高效时段 10:00-12:00', icon: 'bar-chart-outline' },
...
const libraryItems = [
  ...
  { title: '树洞记录', subtitle: '端到端加密，可随时清空', icon: 'lock-closed-outline' },
    ...
    <SafeAreaView style={styles.safeArea}>
      ...
      <ScrollView
        ...
        <View style={styles.header}>
          ...
          <Pressable style={styles.roundButton} accessibilityLabel="打开设置">
        ...
        <View style={styles.companionCard}>
          ...
          <Text style={styles.companionCopy}>你已经为自己留下 28 次专注、11 条心语收藏和 6 个温柔夜晚。</Text>
          <View style={styles.timeline}>
            {['首次打开', '7 日打卡', '深夜陪伴', '今日'].map((item, index) => (
              <View key={item} style={styles.timelineItem}>
                <View style={[styles.timelineDot, index <= 2 && styles.timelineDotDone]} />
        ...
        <Pressable style={styles.shopEntry} accessibilityLabel="进入生活方式选物">
          <View style={styles.shopIcon}>
            <Ionicons name="sparkles-outline" size={20} color={COLORS.orange} />
          </View>
          <View style={styles.shopText}>
            <Text style={styles.shopTitle}>生活方式选物</Text>
            <Text style={styles.shopSubtitle}>低调入口，和声景场景相互呼应</Text>
          </View>
        ...
        <View style={styles.insightList}>
          {insightCards.map((card) => (
            <Pressable key={card.label} style={styles.insightCard} accessibilityLabel={card.label}>
        ...
        <View style={styles.achievementGrid}>
          {achievements.map((achievement) => (
            <View
              ...
              style={[styles.achievementCard, !achievement.active && styles.achievementLocked]}>
              <View style={[styles.achievementIcon, achievement.active && styles.achievementIconActive]}>
                <Ionicons
              ...
              <Text style={[styles.achievementName, !achievement.active && styles.achievementNameLocked]}>
                {achievement.name}
              </Text>
            </View>
          ))}
        ...
        <View style={styles.shareCard}>
          <View>
            ...
            <Text style={styles.shareCopy}>1080 × 1920 竖版图片，仅保留小字 App 标识。</Text>
          </View>
          <Pressable style={styles.shareButton} accessibilityLabel="预览分享卡片">
        ...
        <View style={styles.libraryList}>
          {libraryItems.map((item) => (
            <Pressable key={item.title} style={styles.libraryItem} accessibilityLabel={item.title}>
